...
'
'   （機能名）          仕入単価マスタ保守
'   （クラス名）        frmM50F20_SiireTankaHosyu
'   （処理機能名）      
'   （本MDL使用前提）   
'   （備考）         
'
...
Public Class frmM50F20_SiireTankaHosyu
#Region "宣言"
    ...
    '汎用マスメン
    Private Const lM5020_DB_EXIST As Integer = 1                        'あり：登録済
    Private Const lM5020_DB_NONE As Integer = 0                         'なし：未登録
    ...
    Private _parentForm As frmM50F10_SiireTankaList
...
#Region "コンストラクタ"
    ...
    Public Sub New(ByRef prmRefMsgHd As UtilMsgHandler,
                   ...
                   ByVal prmParentForm As frmM50F10_SiireTankaList,
...
#Region "フォームロード"
    ...
    '-------------------------------------------------------------------------------
    Private Sub frmM50F20_SiireTankaHosyu_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            '操作履歴ログ作成
            Call _comLogc.Insert_L01_ProcLog(_companyCd, DBNull.Value, CommonConst.MENU_M5002, CommonConst.PROGRAM_UPDATE, CommonConst.STATUS_NORMAL,
...
#Region "確定ボタンクリック"
    ...
    Private Sub cmdKautei_Click(sender As Object, e As EventArgs) Handles cmdKakutei.Click
        ...
        Try
            ...
            Select Case _ShoriMode
                ...
                Case CommonConst.MODE_EditStatus   '変更
                    Try
                        sql = ""
                        sql = "SELECT  "
                        sql = sql & N & "    pp.更新日,pp.更新者 "
                        sql = sql & N & " FROM m26_pcprice pp "
                        sql = sql & N & " Where pp.会社コード = '" & frmC01F10_Login.loginValue.BumonCD & "' and pp.商品コード = '" & Me.txtShohinCode.Text & "' and pp.取引先コード = '" & If(Me.txtToriCode.Text = String.Empty, TORICODE_ALL, Me.txtToriCode.Text) & "' and pp.適用開始日 = '" & Me.dtpTekiyoFrDt.Value & "'"
            ...
            Select Case _ShoriMode
                Case CommonConst.MODE_ADDNEW        '新規
                    If M50SIIRETANKA_Add_Ctl() = False Then
                        Exit Sub
                    End If
                Case CommonConst.MODE_ADDNEWCOPY    '複写新規
                    If M50SIIRETANKA_Add_Ctl() = False Then
                    ...
                    End If
                Case CommonConst.MODE_EditStatus    '変更
                    If M50SIIRETANKA_Upd_Ctl() = False Then
                        Exit Sub
                    End If
                Case CommonConst.MODE_DELETE        '削除
                    If M50SIIRETANKA_Del_Ctl() = False Then
                        Exit Sub
                    End If
            End Select
            ...
            '操作履歴ログ作成 
            Call _comLogc.Insert_L01_ProcLog(_companyCd, DBNull.Value, CommonConst.MENU_M5002, CommonConst.PROGRAM_REPORT, CommonConst.STATUS_NORMAL,
...
#Region "プロシージャ"
    ...
    '　データ追加処理
    '-------------------------------------------------------------------------------
    Private Function M50SIIRETANKA_Add_Ctl() As Boolean

        M50SIIRETANKA_Add_Ctl = False
    ...
    '　データ更新処理
    '-------------------------------------------------------------------------------
    Private Function M50SIIRETANKA_Upd_Ctl() As Boolean

        M50SIIRETANKA_Upd_Ctl = False
    ...
    '　データ削除処理
    '-------------------------------------------------------------------------------
    Private Function M50SIIRETANKA_Del_Ctl() As Boolean

        M50SIIRETANKA_Del_Ctl = False
    ...
    '-------------------------------------------------------------------------------
    Private Sub Insert_M50SIIRETANKA()

        Try
            '仕入単価マスタの最新レコード更新
            Dim sql As String = ""
            sql = "UPDATE m26_pcprice"
            sql = sql & N & " SET"
        ...
        Try
            ...
            sql = sql & N & " Where 会社コード = '" & frmC01F10_Login.loginValue.BumonCD & "' and 商品コード = '" & Me.txtShohinCode.Text & "' and 取引先コード = '" & If(Me.txtToriCode.Text = String.Empty, TORICODE_ALL, Me.txtToriCode.Text) & "' and 適用開始日 >= '" & Me.dtpTekiyoFrDt.Value & "'"
            Dim reccnt As Integer = 0
            Dim ds As DataSet = _db.selectDB(sql, RS, reccnt)
            If reccnt = 0 Then
            ...
            sql = sql & N & ", " & nullToNum(Me.txtSiireTanka.Value)
    ...
    '　（処理概要）sql文を作成し，ＤＢへの登録を行う
    '-------------------------------------------------------------------------------
    Private Sub Delete_M50SIIRETANKA()
        ...
        Try
            ...
            sql = ""
            sql = "SELECT "
            sql = sql & N & "    適用開始日, 適用終了日, 仕入単価, メモ "
            sql = sql & N & " FROM m26_pcprice "
            sql = sql & N & " Where 会社コード = '" & frmC01F10_Login.loginValue.BumonCD & "' and 商品コード = '" & Me.txtShohinCode.Text & "' and 取引先コード = '" & If(Me.txtToriCode.Text = String.Empty, TORICODE_ALL, Me.txtToriCode.Text) & "' and 適用終了日 < '" & Me.txtTekiyoFrDt.Text & "'"
            ...
            Try
                ...
                If iRecCnt = 0 Then
                    ...
                    sql = sql & N & "    min(適用開始日) 開始日 "
                    sql = sql & N & " FROM m26_pcprice "
                    sql = sql & N & " Where 会社コード = '" & frmC01F10_Login.loginValue.BumonCD & "' and 商品コード = '" & Me.txtShohinCode.Text & "' and 取引先コード = '" & If(Me.txtToriCode.Text = String.Empty, TORICODE_ALL, Me.txtToriCode.Text) & "' and 適用開始日 > '" & Me.txtTekiyoToDt.Text & "'"
                    ...
                    If nullToStr(oDataSet2.Tables(RS).Rows(0)("開始日")) <> "NULL" Then
                        '適用開始日はキー項目なのでDELETE／INSERTするためにDELETE前に該当データからキー項目以外の項目（仕入単価・適用終了日・メモ）を取得しておく
                        tekiyoKaisiDate = DateTime.Parse(oDataSet2.Tables(RS).Rows(0)("開始日")).ToString("yyyy/MM/dd")
                        sql = ""
                        sql = "SELECT "
                        sql = sql & N & "    仕入単価, 適用終了日, メモ "
                        sql = sql & N & " FROM m26_pcprice "
                        sql = sql & N & " Where 会社コード = '" & frmC01F10_Login.loginValue.BumonCD & "' and 商品コード = '" & Me.txtShohinCode.Text & "' and 取引先コード = '" & If(Me.txtToriCode.Text = String.Empty, TORICODE_ALL, Me.txtToriCode.Text) & "' and 適用開始日 = '" & tekiyoKaisiDate & "'"
                        Dim oDataSet3 As DataSet = _db.selectDB(sql, RS, iRecCnt3)    '抽出結果をDSへ格納
                        If iRecCnt3 <> 0 Then
                            siireTanka = _db.rmNullDouble(oDataSet3.Tables(RS).Rows(0)("仕入単価"))
                        ...
                        Try
                            ...
                            sql = sql & N & ", '" & tekiyoShuryoDate & "'"
                            sql = sql & N & ", " & nullToNum(siireTanka)
    ...
    '　（処理概要）sql文を作成し，ＤＢへの登録を行う
    '-------------------------------------------------------------------------------
    Private Sub Update_M50SIIRETANKA()
    ...
    Private Sub getMasterData()
        ...
        Try
            sql = "SELECT "
            sql = sql & N & "  pp.商品コード, pp.取引先コード, pp.適用開始日, pp.適用終了日, pp.仕入単価, pp.メモ, pp.更新者, pp.更新日 "
            sql = sql & N & " ,g.商品名 商品名 "
            ...
            sql = sql & N & " Where pp.会社コード = '" & frmC01F10_Login.loginValue.BumonCD & "' and pp.商品コード = '" & shohinCode & "' and pp.取引先コード = '" & toriCode & "' and pp.適用開始日 = '" & tekiyoFrDT & "'"
            ...
            End If
            Me.txtSiireTanka.Text = _db.rmNullDouble(ds.Tables(RS).Rows(0)("仕入単価"))
    ...
    Private Sub initializeDisplay()
        ...
        Me.txtSiireTanka.Text = String.Empty
    ...
    Private Sub setBackColor()
        ...
        Select Case _ShoriMode
            ...
            Case CommonConst.MODE_InquiryStatus, CommonConst.MODE_DELETE        '参照,削除
                ...
                Me.txtTekiyoFrDt.BackColor = _readOnlyTextBackColor
                Me.txtTekiyoToDt.BackColor = _readOnlyTextBackColor
                Me.txtSiireTanka.BackColor = _readOnlyTextBackColor
    ...
    Private Sub setProperty()
        ...
        Select Case _ShoriMode
            ...
            Case CommonConst.MODE_ADDNEW, CommonConst.MODE_ADDNEWCOPY           '新規追加,複写新規
                ...
                Me.txtSiireTanka.ReadOnly = False
