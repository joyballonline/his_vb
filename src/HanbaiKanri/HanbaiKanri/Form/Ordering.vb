...
Public Class Ordering
    ...
    Private Sub Ordering_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        DgvItemList.Columns("仕入単価").HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleCenter
        DgvItemList.Columns("仕入原価_外貨").HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleCenter
        ...
        If frmC01F10_Login.loginValue.Language = CommonConst.LANG_KBN_ENG Then
            ...
            lblPurchasecost2.Text = "PurchaseCost"      '仕入原価(原通貨)
        ...
        Else
            ...
            DgvItemList.Columns("仕入原価").HeaderText = "仕入原価(" & setBaseCurrency() & ")" & vbCrLf & "c=a*b"
        ...
        For i As Integer = 0 To dsHattyudt.Tables(RS).Rows.Count - 1
            ...
            DgvItemList.Rows(i).Cells("仕入原価_外貨").Value = Decimal.Parse(dsHattyudt.Tables(RS).Rows(i)("仕入値_外貨") * dsHattyudt.Tables(RS).Rows(i)("発注数量"))
            DgvItemList.Rows(i).Cells("仕入原価").Value = Decimal.Parse(dsHattyudt.Tables(RS).Rows(i)("仕入値") * dsHattyudt.Tables(RS).Rows(i)("発注数量"))

            DgvItemList.Rows(i).Cells("関税率").Value = dsHattyudt.Tables(RS).Rows(i)("関税率") * 100
            ...
            decOverhead2 += DgvItemList.Rows(i).Cells("仕入金額_外貨").Value
        ...
        txtPurchasecost2.Text = Decimal.Parse(tmp_cur2).ToString("N2")        '仕入原価_外貨
        TxtPurchaseAmount2.Text = Decimal.Parse(decOverhead2).ToString("N2")  '仕入金額_外貨
    ...
    Private Sub CellValueChanged(ByVal sender As Object, ByVal e As DataGridViewCellEventArgs) Handles DgvItemList.CellValueChanged
        ...
        If e.RowIndex > -1 Then
            ...
            '発注金額をクリア
            txtPurchasecost.Text = 0.ToString("N2")
            TxtPurchaseAmount.Text = 0.ToString("N2")
            txtPurchasecost.Text = 0.ToString("N2")
            TxtPurchaseAmount2.Text = 0.ToString("N2")
            ...
            If DgvItemList.Rows(e.RowIndex).Cells("仕入単価").Value IsNot Nothing Then
                ...
                If DgvItemList.Rows(e.RowIndex).Cells("数量").Value IsNot Nothing Then
                    DgvItemList.Rows(e.RowIndex).Cells("仕入原価_外貨").Value = DgvItemList.Rows(e.RowIndex).Cells("仕入単価_外貨").Value * DgvItemList.Rows(e.RowIndex).Cells("数量").Value
                ...
                If DgvItemList.Rows(e.RowIndex).Cells("仕入原価").Value IsNot Nothing Then
                    ...
                    Else
                        DgvItemList.Rows(e.RowIndex).Cells("仕入金額_外貨").Value = DgvItemList.Rows(e.RowIndex).Cells("仕入金額").Value * TxtRate.Text
                    End If
                ...
                If currentColumn = "仕入単価" Then
                    '仕入単価_外貨
                    DgvItemList("仕入単価_外貨", e.RowIndex).Value = DgvItemList("仕入単価", e.RowIndex).Value / TxtRate.Text
                End If
    ...
    Private Sub BtnRegistration_Click(sender As Object, e As EventArgs) Handles BtnRegistration.Click
        ...
        Try
            ...
            If PurchaseStatus = CommonConst.STATUS_CLONE Or PurchaseStatus = CommonConst.STATUS_EDIT Or PurchaseStatus = CommonConst.STATUS_ADD Then
                ...
                Sql += "0" '見積金額
                Sql += "', '"
                Sql += UtilClass.formatNumber(TxtPurchaseAmount.Text) '仕入金額
                Sql += "', '"
                Sql += "0" '粗利額
                Sql += "', '"
                ...
                Sql += "', '"
                Sql += UtilClass.formatNumber(TxtPurchaseAmount2.Text)  '仕入金額_外貨
                ...
                For i As Integer = 0 To DgvItemList.Rows.Count - 1
                    ...
                    Sql += "', '"
                    Sql += UtilClass.formatNumber(DgvItemList.Rows(i).Cells("仕入単価").Value.ToString) '仕入値
                    Sql += "', '"
                    Sql += UtilClass.formatNumber(DgvItemList.Rows(i).Cells("数量").Value.ToString) '発注数量
                    Sql += "', '"
                    Sql += "0" '仕入数量
                    Sql += "', '"
                    Sql += UtilClass.formatNumber(DgvItemList.Rows(i).Cells("数量").Value.ToString) '発注残数
                    Sql += "', 0" '間接費
                    Sql += ", '"
                    Sql += UtilClass.formatNumber(DgvItemList.Rows(i).Cells("仕入金額").Value.ToString) '仕入金額
                    Sql += "', '"
                    Sql += IIf(
                    ...
                    Sql += ", current_timestamp" '更新日
                    Sql += ", '"
                    Sql += UtilClass.formatNumber(DgvItemList.Rows(i).Cells("仕入単価_外貨").Value) '仕入単価_外貨
                    Sql += "', '"
                    Sql += CmCurrency.SelectedValue.ToString '仕入通貨
                    Sql += "', '"
                    Sql += UtilClass.formatNumberF10(strRate) '仕入レート  発注日で計算し直したデータを入れる
                    Sql += "', '"
                    Sql += UtilClass.formatNumber(DgvItemList.Rows(i).Cells("関税率").Value / 100) '関税率
                    Sql += "', '"
                    Sql += UtilClass.formatNumber(DgvItemList.Rows(i).Cells("関税額").Value) '関税額
                    Sql += "', '"
                    Sql += UtilClass.formatNumber(DgvItemList.Rows(i).Cells("前払法人税率").Value / 100) '前払法人税率
                    Sql += "', '"
                    Sql += UtilClass.formatNumber(DgvItemList.Rows(i).Cells("前払法人税額").Value) '前払法人税額
                    Sql += "', '"
                    Sql += UtilClass.formatNumber(DgvItemList.Rows(i).Cells("輸送費率").Value / 100) '輸送費率
                    Sql += "', '"
                    Sql += UtilClass.formatNumber(DgvItemList.Rows(i).Cells("輸送費額").Value) '輸送費額

                    Sql += "', "
                    Sql += UtilClass.formatNumber(DgvItemList.Rows(i).Cells("仕入単価_外貨").Value)  '仕入値_外貨
                    Sql += ", "
    ...
    Private Sub setCurrency()
        ...
        Dim Purchasecost2 As Decimal = 0
        Dim PurchaseTotal2 As Decimal = 0
        ...
        For c As Integer = 0 To DgvItemList.Rows.Count - 1
            ...
            Purchasecost2 += DgvItemList.Rows(c).Cells("仕入原価_外貨").Value
            PurchaseTotal2 += DgvItemList.Rows(c).Cells("仕入金額_外貨").Value
        Next

        txtPurchasecost.Text = Purchasecost.ToString("N2")     '仕入原価
        TxtPurchaseAmount.Text = PurchaseTotal.ToString("N2")

        txtPurchasecost2.Text = Purchasecost2.ToString("N2")
        TxtPurchaseAmount2.Text = PurchaseTotal2.ToString("N2")
        ...
        If DgvItemList.Rows.Count > 0 Then
            For i As Integer = 0 To DgvItemList.Rows.Count - 1
                ...
                If DgvItemList.Rows(i).Cells("仕入単価_外貨").Value IsNot Nothing And DgvItemList.Rows(i).Cells("数量").Value IsNot Nothing Then
                    ...
                    DgvItemList.Rows(i).Cells("仕入原価_外貨").Value = DgvItemList.Rows(i).Cells("仕入単価_外貨").Value * DgvItemList.Rows(i).Cells("数量").Value


                    Dim dotAlign As Decimal = DgvItemList.Rows(i).Cells("仕入単価_外貨").Value / TxtRate.Text
                    dotAlign = Math.Ceiling(dotAlign * 100) / 100

                    DgvItemList.Rows(i).Cells("仕入単価").Value = dotAlign
                    DgvItemList.Rows(i).Cells("仕入原価").Value = DgvItemList.Rows(i).Cells("仕入単価").Value * DgvItemList.Rows(i).Cells("数量").Value


                    '関税率
                    If DgvItemList.Rows(i).Cells("関税率").Value IsNot Nothing Then
                        DgvItemList.Rows(i).Cells("関税額").Value = DgvItemList.Rows(i).Cells("仕入単価").Value * DgvItemList.Rows(i).Cells("関税率").Value / 100
                    End If
                    ...
                    If DgvItemList.Rows(i).Cells("前払法人税率").Value IsNot Nothing And DgvItemList.Rows(i).Cells("関税額").Value IsNot Nothing Then
                        ...
                        Dim tmp1 As Decimal = tmp * DgvItemList.Rows(i).Cells("前払法人税率").Value / 100
                        DgvItemList.Rows(i).Cells("前払法人税額").Value = tmp1
                    End If

                    '輸送費率
                    If DgvItemList.Rows(i).Cells("輸送費率").Value IsNot Nothing Then
                        DgvItemList.Rows(i).Cells("輸送費額").Value = DgvItemList.Rows(i).Cells("仕入単価").Value * DgvItemList.Rows(i).Cells("輸送費率").Value / 100
                    End If
                    ...
                    Else
                        DgvItemList.Rows(i).Cells("仕入金額_外貨").Value = DgvItemList.Rows(i).Cells("仕入金額").Value * TxtRate.Text
                    End If
    ...
    Private Sub setSireTax(ByVal prmRowIndex As Integer)
        ...
        If ds.Tables(RS).Rows.Count > 0 Then
            DgvItemList.Rows(prmRowIndex).Cells("関税率").Value = ds.Tables(RS).Rows(0)("関税率").ToString * 100
            DgvItemList.Rows(prmRowIndex).Cells("前払法人税率").Value = ds.Tables(RS).Rows(0)("前払法人税率").ToString * 100
            DgvItemList.Rows(prmRowIndex).Cells("輸送費率").Value = ds.Tables(RS).Rows(0)("輸送費率").ToString * 100
        Else
    ...
    Private Sub TxtSupplierName_TextChanged(sender As Object, e As EventArgs) Handles TxtSupplierName.TextChanged
        ...
        For i As Integer = 0 To DgvItemList.Rows.Count - 1
            DgvItemList.Rows(i).Cells("関税率").Value = ds.Tables(RS).Rows(0)("関税率").ToString * 100
            DgvItemList.Rows(i).Cells("前払法人税率").Value = ds.Tables(RS).Rows(0)("前払法人税率").ToString * 100
            DgvItemList.Rows(i).Cells("輸送費率").Value = ds.Tables(RS).Rows(0)("輸送費率").ToString * 100
        Next
