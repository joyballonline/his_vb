...
Public Class frmH10F04_TokuisakiMotoList
    '-------------------------------------------------------------------------------
    '   定数定義
    '-------------------------------------------------------------------------------
    'PG制御文字 
...
#Region "フォームロード"
    ...
    '-------------------------------------------------------------------------------
    Private Sub frmH10F04_TokuisakiMotoList_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
...
#Region "プレビューボタンクリック"
    ...
    Private Sub cmdPreview_Click(sender As Object, e As EventArgs) Handles cmdPreview.Click
        ...
        Try
            ...
            '注文明細表
            Dim rSect As RepSectionIF = New H10R04_TokuisakiMotoList
...
#Region "出荷先分類Click時"
    ...
    Private Sub rbItaku_Click(sender As Object, e As EventArgs) Handles rbItaku.Click,
        ...
        If rbUriage.Checked = True Then
            grpItaku.Enabled = False
        Else
            grpItaku.Enabled = True
        End If
...
#Region "売上入金年月Leave時"
    ...
    '　売上入金年月Leave時、日付設定
    '-------------------------------------------------------------------------------
    Private Sub txtNyukinYm_Leave(sender As Object, e As EventArgs) Handles txtNyukinYm.Leave

        If _frmOpen = False Then Exit Sub                            '起動中は実行しない

        If txtNyukinYm.Text = DATE_EMPTY Then Exit Sub
...
#End Region
    ...
    Private Function check_Input() As Boolean
        ...
        '年月チェック
        If Not txtNyukinYm.Text.Equals(dtYmdFrom.Text.Substring(0, 7)) Then
            piRtn = _msgHd.dspMSG("notUriageYm", "")                                '売上入金年月と違います。
            dtYmdFrom.Focus()
            Exit Function
        End If
        If Not txtNyukinYm.Text.Equals(dtYmdTo.Text.Substring(0, 7)) Then
            piRtn = _msgHd.dspMSG("notUriageYm", "")                                '売上入金年月と違います。
            dtYmdTo.Focus()
            Exit Function
        End If
    ...
    Private Function makeSql() As String
        ...
        If rbItakuYes.Checked = True Then
            sSql = sSql & " ,'" & rbItakuYes.Text & "' 条件_委託残情報"
        Else
            sSql = sSql & " ,'" & rbItakuNo.Text & "' 条件_委託残情報"
        End If
        ...
        sSql = sSql & "             ,t20.請求先コード  請求先コード"
        sSql = sSql & "             ,t20.請求先名      請求先名"
        sSql = sSql & "             ,t20.売上日        売上日"
        sSql = sSql & "             ,CASE WHEN t20.出荷先分類 = '" & CommonConst.SKBUNRUI_ITAKU & "'"
        sSql = sSql & "                   THEN t20.売上伝番 || '-' || t20.売上伝番枝番"
        sSql = sSql & "                   ELSE t20.売上伝番"
        sSql = sSql & "              END               伝票番号"
        sSql = sSql & "             ,t21.行番          行番"
        sSql = sSql & "             ,t20.売上区分      区分"
        sSql = sSql & "             ,t20.出荷先名      出荷先"
        sSql = sSql & "             ,CASE WHEN t16.委託残数 <> 0"
        sSql = sSql & "                   THEN '◎'"
        sSql = sSql & "                   ELSE ''"
        sSql = sSql & "              END               委託残あり表示"
        sSql = sSql & "             ,t21.商品名 || COALESCE(t21.荷姿形状,'') 商品入金摘要"       'NVL
        sSql = sSql & "             ,t21.売上数量      数量"
        sSql = sSql & "             ,t21.単位          単位"
        sSql = sSql & "             ,t21.売上単価      単価"
        sSql = sSql & "             ,t21.売上金額      売上額"
        sSql = sSql & "             ,CASE WHEN t21.行番 = 1"
        sSql = sSql & "                   THEN t20.消費税計"
        sSql = sSql & "                   ELSE NULL"
        sSql = sSql & "              END               消費税"
        sSql = sSql & "             ,NULL              入金額"
        sSql = sSql & "             ,0                 残高"
        ...
        sSql = sSql & "       LEFT JOIN t21_urigdt As t21    On t21.会社コード   = t20.会社コード"
        sSql = sSql & "                                     And t21.売上伝番     = t20.売上伝番"
        sSql = sSql & "                                     And t21.売上伝番枝番 = t20.売上伝番枝番"
        sSql = sSql & "       LEFT JOIN t16_itakdt As t16    On t16.会社コード   = t21.会社コード"
        sSql = sSql & "                                     And t16.委託伝番     = t21.売上伝番"
        sSql = sSql & "                                     And t16.行番         = t21.注文行番"
        ...
        If rbItaku.Checked = True Then
            sSql = sSql & "     AND t20.出荷先分類   = '" & CommonConst.SKBUNRUI_ITAKU & "'"        '委託
        ElseIf rbUriage.Checked = True Then
            sSql = sSql & "     AND t20.出荷先分類   = '" & CommonConst.SKBUNRUI_URIAGE & "'"       '売上
        End If
        sSql = sSql & "     UNION ALL"
        sSql = sSql & "       SELECT 3                 取得元テーブル"
        sSql = sSql & "             ,t25.請求先コード  請求先コード"
        sSql = sSql & "             ,t25.請求先名      請求先名"
        sSql = sSql & "             ,t25.入金日        売上日"
        sSql = sSql & "             ,t25.入金伝番      伝票番号"
        sSql = sSql & "             ,t26.行番          行番"
        sSql = sSql & "             ,'入金'            区分"
        'sSql = sSql & "             ,NULL              出荷先"
        sSql = sSql & "             ,t26.入金種別名    出荷先"
        sSql = sSql & "             ,NULL              委託残あり表示"
        'sSql = sSql & "             ,CASE WHEN t26.行番 = 1"
        'sSql = sSql & "                   THEN t26.入金種別名 || ' ' ||COALESCE( t25.備考１,'')"
        'sSql = sSql & "                   ELSE t26.入金種別名 "
        'sSql = sSql & "              END               商品入金摘要"
        sSql = sSql & "             ,CASE WHEN t26.行番 = 1"
        sSql = sSql & "                   THEN COALESCE( t25.備考１,'')"
        sSql = sSql & "                   ELSE '' "
        sSql = sSql & "              END               商品入金摘要"
        sSql = sSql & "             ,NULL              数量"
        sSql = sSql & "             ,NULL              単位"
        ...
        sSql = sSql & "             ,NULL              売上額"
        sSql = sSql & "             ,NULL              消費税"
        sSql = sSql & "             ,t26.金額          入金額"
        sSql = sSql & "             ,0                 残高"
        sSql = sSql & "             ,m10.出荷先分類"
        sSql = sSql & "             ,m10.取引先名カナ"
        sSql = sSql & "             ,m01.会社名"
        sSql = sSql & "       FROM t25_nkinhd t25"      '入金基本
        sSql = sSql & "       LEFT JOIN t26_nkindt As t26    On t26.会社コード   = t25.会社コード"
        sSql = sSql & "                                     And t26.入金伝番     = t25.入金伝番"
        ...
        sSql = sSql & "         AND TO_CHAR(t25.入金日,'YYYY/MM/DD') >= '" & dtYmdFrom.Text & "'"
        sSql = sSql & "         AND TO_CHAR(t25.入金日,'YYYY/MM/DD') <= '" & dtYmdTo.Text & "'"
        If rbItaku.Checked = True Then
            sSql = sSql & "     AND m10.出荷先分類   = '" & CommonConst.SKBUNRUI_ITAKU & "'"        '委託
        ElseIf rbUriage.Checked = True Then
            sSql = sSql & "     AND m10.出荷先分類   = '" & CommonConst.SKBUNRUI_URIAGE & "'"       '売上
        End If
        sSql = sSql & "     UNION ALL"
        sSql = sSql & "       SELECT 1                取得元テーブル"
        sSql = sSql & "             ,t30.請求先コード"
        sSql = sSql & "             ,t30.請求先名"
        sSql = sSql & "             ,TO_DATE(年月 || '01', 'YYYYMMDD') 売上日"
        sSql = sSql & "             ,NULL             伝票番号"
        sSql = sSql & "             ,NULL             行番"
        sSql = sSql & "             ,NULL             区分"
        sSql = sSql & "             ,'" & STRING_KURIKOSIGAKU & "'     出荷先"
        sSql = sSql & "             ,NULL             委託残あり表示"
        sSql = sSql & "             ,NULL             商品入金摘要"
        sSql = sSql & "             ,NULL             数量"
        sSql = sSql & "             ,NULL             単位"
        sSql = sSql & "             ,NULL             単価"
        sSql = sSql & "             ,NULL             売上額"
        ...
        sSql = sSql & "      ) AS  AA "
        If rbSortCode.Checked = True Then
            sSql = sSql & " ORDER BY AA.出荷先分類, AA.請求先コード, AA.売上日, AA.取得元テーブル, AA.伝票番号, AA.行番"
        Else
            sSql = sSql & " ORDER BY AA.出荷先分類, AA.取引先名カナ, AA.請求先コード, AA.売上日, AA.取得元テーブル, AA.伝票番号, AA.行番"
        End If
    ...
    Private Function add_Kurikosi(ByVal paraDs As DataSet) As DataSet
        ...
        dt.Columns.Add("条件_委託残情報", GetType(String))
        dt.Columns.Add("条件_出力順", GetType(String))
        dt.Columns.Add("条件_ログ用", GetType(String))

        dt.Columns.Add("取得元テーブル", GetType(String))
        dt.Columns.Add("請求先コード", GetType(String))
        dt.Columns.Add("請求先名", GetType(String))
        dt.Columns.Add("売上日", GetType(Date))
        dt.Columns.Add("伝票番号", GetType(String))
        dt.Columns.Add("行番", GetType(Int32))
        dt.Columns.Add("区分", GetType(String))
        dt.Columns.Add("出荷先", GetType(String))
        dt.Columns.Add("委託残あり表示", GetType(String))
        dt.Columns.Add("商品入金摘要", GetType(String))
        dt.Columns.Add("数量", GetType(Decimal))
        dt.Columns.Add("単位", GetType(String))
        dt.Columns.Add("単価", GetType(Decimal))
        dt.Columns.Add("売上額", GetType(Decimal))
        dt.Columns.Add("消費税", GetType(Decimal))
        dt.Columns.Add("入金額", GetType(Decimal))
        dt.Columns.Add("残高", GetType(Decimal))
        dt.Columns.Add("出荷先分類", GetType(String))
        dt.Columns.Add("取引先名カナ", GetType(String))
        dt.Columns.Add("会社名", GetType(String))

        Dim sSeikyusakiCd As String = ""
        For index As Integer = 0 To paraDs.Tables(RS).Rows.Count - 1
            Dim newRow As DataRow = dt.NewRow

            '請求先コードがkeyブレイクで債権残高(t30)レコードがない場合
            If Not sSeikyusakiCd.Equals(_db.rmNullStr(paraDs.Tables(RS).Rows(index)("請求先コード"))) AndAlso
              Not _db.rmNullStr(paraDs.Tables(RS).Rows(index)("取得元テーブル")).Equals("1") Then
                ...
                newRow("条件_委託残情報") = _db.rmNullStr(paraDs.Tables(RS).Rows(index)("条件_委託残情報"))
                newRow("条件_出力順") = _db.rmNullStr(paraDs.Tables(RS).Rows(index)("条件_出力順"))
                newRow("条件_ログ用") = _db.rmNullStr(paraDs.Tables(RS).Rows(index)("条件_ログ用"))

                newRow("取得元テーブル") = "1"
                newRow("請求先コード") = _db.rmNullStr(paraDs.Tables(RS).Rows(index)("請求先コード"))
                newRow("請求先名") = _db.rmNullStr(paraDs.Tables(RS).Rows(index)("請求先名"))
                Dim dUriage As Date = DateTime.Parse(_db.rmNullStr(paraDs.Tables(RS).Rows(index)("売上日")).Substring(0, 7) & "/01")
                dUriage = dUriage.AddDays(-1)
                newRow("売上日") = dUriage.ToShortDateString
                newRow("伝票番号") = ""
                newRow("行番") = 0
                newRow("区分") = ""
                newRow("出荷先") = STRING_KURIKOSIGAKU
                newRow("委託残あり表示") = ""
                newRow("商品入金摘要") = ""
                newRow("数量") = 0
                newRow("単位") = ""
                ...
                newRow("入金額") = 0
                newRow("残高") = 0
                newRow("出荷先分類") = _db.rmNullStr(paraDs.Tables(RS).Rows(index)("出荷先分類"))
            ...
            End If
            'keyブレイク用
            sSeikyusakiCd = _db.rmNullStr(paraDs.Tables(RS).Rows(index)("請求先コード"))
            ...
            newRow("請求先コード") = _db.rmNullStr(paraDs.Tables(RS).Rows(index)("請求先コード"))
            newRow("請求先名") = _db.rmNullStr(paraDs.Tables(RS).Rows(index)("請求先名"))
            newRow("売上日") = _db.rmNullStr(paraDs.Tables(RS).Rows(index)("売上日"))
            newRow("伝票番号") = _db.rmNullStr(paraDs.Tables(RS).Rows(index)("伝票番号"))
            newRow("行番") = _db.rmNullDouble(paraDs.Tables(RS).Rows(index)("行番"))
            newRow("区分") = _db.rmNullStr(paraDs.Tables(RS).Rows(index)("区分"))
            newRow("出荷先") = _db.rmNullStr(paraDs.Tables(RS).Rows(index)("出荷先"))
            newRow("委託残あり表示") = _db.rmNullStr(paraDs.Tables(RS).Rows(index)("委託残あり表示"))
            newRow("商品入金摘要") = _db.rmNullStr(paraDs.Tables(RS).Rows(index)("商品入金摘要"))
            ...
            newRow("売上額") = _db.rmNullDouble(paraDs.Tables(RS).Rows(index)("売上額"))
            newRow("消費税") = _db.rmNullDouble(paraDs.Tables(RS).Rows(index)("消費税"))
            newRow("入金額") = _db.rmNullDouble(paraDs.Tables(RS).Rows(index)("入金額"))
