...
#Region "行タイトル"
        ...
        If frmC01F10_Login.loginValue.Language = CommonConst.LANG_KBN_ENG Then  '英語 
            ...
            DgvList.Columns("請求番号").HeaderText = "INVOICE NO. ZMEI" 'A
            DgvList.Columns("請求日").HeaderText = "DATE" 'B
            ...
            DgvList.Columns("受注数量").HeaderText = "QTY" & vbCrLf & "b" 'C
            ...
            DgvList.Columns("仕入単価_原通貨").HeaderText = "AMOUNT" & vbCrLf & "z=b*(PurchaseUnitPrice)" 'Q
            ...
            DgvList.Columns("利益").HeaderText = "PROFIT" & vbCrLf & "g=x-z-f" 'R
...
#Region "SQL"
        ...
        Sql += " AND "
        Sql += " t23.請求日 <= '" & UtilClass.strFormatDate(DateTimePicker2.Value) & "'"
        Sql += " ORDER BY t23.請求日,t23.売上行番号 "
#End Region
        ...
        Try
            ...
            For i As Integer = 0 To ds.Tables(RS).Rows.Count - 1
                ...
                ElseIf (fkpc_ = CommonConst.Sire_KBN_Sire) Or (fkpc_ = CommonConst.Sire_KBN_DELIVERY) Then
                    Sql = "SELECT t21.*, t20.* "
                    Sql += " FROM t21_hattyu t21, t20_hattyu t20 "
                    Sql += " WHERE "
                    Sql += " t20.受注番号 = '" & ds.Tables(RS).Rows(i)("受注番号") & "'"
                    ...
                    Else
                        ...
                        DgvList.Rows(intListCnt).Cells("仕入先名").Value = ds2.Tables(RS).Rows(ii)("仕入先名")
                        'DgvList.Rows(intListCnt).Cells("仕入単価_IDR").Value = UtilClass.rmNullDecimal(ds2.Tables(RS).Rows(ii)("仕入値"))
                        DgvList.Rows(intListCnt).Cells("仕入単価_原通貨").Value = DgvList.Rows(intListCnt).Cells("受注数量").Value * UtilClass.rmNullDecimal(ds2.Tables(RS).Rows(ii)("仕入値_外貨"))
                        'DgvList.Rows(intListCnt).Cells("仕入原価_原通貨").Value = UtilClass.rmNullDecimal(ds2.Tables(RS).Rows(ii)("仕入金額_外貨"))
                        'DgvList.Rows(intListCnt).Cells("仕入原価_IDR").Value = UtilClass.rmNullDecimal(ds2.Tables(RS).Rows(ii)("仕入金額"))
                        DgvList.Rows(intListCnt).Cells("間接費").Value = ds2.Tables(RS).Rows(ii)("間接費")
                        DgvList.Rows(intListCnt).Cells("得意先コード").Value = invd

                        DgvList.Rows(intListCnt).Cells("利益").Value = DgvList.Rows(intListCnt).Cells("受注単価_原通貨").Value - DgvList.Rows(intListCnt).Cells("仕入単価_原通貨").Value - DgvList.Rows(intListCnt).Cells("間接費").Value
...
#End Region
    ...
    Private Sub outputExcel3()
        ...
        Try
            ...
            rowCnt = DgvList.Rows.Count - 1
            Dim array2(DgvList.RowCount - 1, (18 - 1)) As String
            ...
            For i As Integer = 0 To DgvList.RowCount - 1
                ...
                'sheet.Range("D" & cellRowIndex.ToString).Value = DgvList.Rows(i).Cells("売上番号").Value '売上番号
                'sheet.Range("E" & cellRowIndex.ToString).Value = DgvList.Rows(i).Cells("売上番号枝番").Value '売上番号枝番
                array2(i, 0) = DgvList.Rows(i).Cells("請求番号").Value '請求番号 A
                array2(i, 1) = DgvList.Rows(i).Cells("請求日").Value '請求日 B

                array2(i, 12) = DgvList.Rows(i).Cells("得意先コード").Value '得意先コード M
                array2(i, 7) = DgvList.Rows(i).Cells("得意先名").Value '得意先名 H

                'array2(i, 8) = DgvList.Rows(i).Cells("メーカー").Value 'メーカー
                'array2(i, 9) = DgvList.Rows(i).Cells("品名").Value '品名
                'array2(i, 10) = DgvList.Rows(i).Cells("型式").Value '型式

                array2(i, 3) = DgvList.Rows(i).Cells("販売通貨").Value '販売通貨 D
                array2(i, 4) = UtilClass.rmNullDecimal(DgvList.Rows(i).Cells("受注単価_原通貨").Value).ToString("N2") '受注単価_原通貨 E
                'array2(i, 13) = UtilClass.rmNullDecimal(DgvList.Rows(i).Cells("受注単価_IDR").Value).ToString("N2") '受注単価_IDR
                array2(i, 2) = UtilClass.rmNullDecimal(DgvList.Rows(i).Cells("受注数量").Value).ToString("N2") '受注数量 C
                array2(i, 8) = DgvList.Rows(i).Cells("単位").Value '単位 I

                array2(i, 6) = UtilClass.rmNullDecimal(DgvList.Rows(i).Cells("受注金額_原通貨").Value).ToString("N2") '受注金額_原通貨 G
                array2(i, 5) = UtilClass.rmNullDecimal(DgvList.Rows(i).Cells("受注金額_IDR").Value).ToString("N2") '受注金額_IDR F

                array2(i, 9) = DgvList.Rows(i).Cells("発注番号").Value '発注番号 J
                'array2(i, 19) = DgvList.Rows(i).Cells("発注番号枝番").Value '発注番号枝番
                array2(i, 13) = DgvList.Rows(i).Cells("発注行番号").Value '発注行番号 N

                'sheet.Range("U" & cellRowIndex.ToString).Value = DgvList.Rows(i).Cells("仕入番号").Value '仕入番号
                'sheet.Range("V" & cellRowIndex.ToString).Value = DgvList.Rows(i).Cells("仕入行番号").Value '仕入行番号
                array2(i, 14) = DgvList.Rows(i).Cells("仕入区分").Value '仕入区分 O

                array2(i, 11) = DgvList.Rows(i).Cells("仕入先コード").Value '仕入先コード L
                array2(i, 10) = DgvList.Rows(i).Cells("仕入先名").Value '仕入先名 K

                array2(i, 15) = DgvList.Rows(i).Cells("仕入通貨").Value '仕入通貨 P
                array2(i, 16) = UtilClass.rmNullDecimal(DgvList.Rows(i).Cells("仕入単価_原通貨").Value).ToString("N2") '仕入単価_原通貨 Q
                'array2(i, 26) = UtilClass.rmNullDecimal(DgvList.Rows(i).Cells("仕入単価_IDR").Value).ToString("N2") '仕入単価_IDR
                'array2(i, 27) = UtilClass.rmNullDecimal(DgvList.Rows(i).Cells("仕入原価_原通貨").Value).ToString("N2") '仕入原価_原通貨
                'array2(i, 28) = UtilClass.rmNullDecimal(DgvList.Rows(i).Cells("仕入原価_IDR").Value).ToString("N2") '仕入原価_IDR

                'array2(i, 29) = UtilClass.rmNullDecimal(DgvList.Rows(i).Cells("間接費").Value).ToString("N2") '間接費
                array2(i, 17) = UtilClass.rmNullDecimal(DgvList.Rows(i).Cells("利益").Value).ToString("N2") '利益 R
                'array2(i, 31) = UtilClass.rmNullDecimal(DgvList.Rows(i).Cells("利益率").Value).ToString("N1") '利益率

            Next

            sheet.Range("A5:R" & (DgvList.RowCount + 1 + 3).ToString).Value = array2
            'sheet.Columns("A:AF").EntireColumn.AutoFit  '幅の自動調整
